...
# Skill: 8-dev-frontend

This skill implements the frontend user interface and client logic in an isolated Git worktree based on the merged backend, following the UI/UX design (Step 3) and frontend specifications (Step 4). It verifies the implementation with automated tests and opens a Pull Request configured to automatically close the parent issue upon merge (`Closes #<id>`).
...
Locate issues tagged with `dev-frontend` (advanced by `7-merge-backend`):

```bash
gh issue list --label "dev-frontend"
```

Read the target issue for design and frontend technical specifications:
...
Carefully review:
- **UI/UX Design**: Screens, layouts, responsive behavior, and states (loading, error, empty, nominal) from Step 3.
- **Frontend Technical Specs**: Component breakdown, state management, and backend endpoints to call from Step 4.
...
- Detect the frontend framework and tooling (or read `ARCHITECTURE.md`).
- Build components matching the structure and states designed in Step 3.
- Implement API service integration connecting to the backend endpoints verified in Step 7.
...
## Deliverables
- [x] UI/UX components matching designs (Step 3)
- [x] Integration with backend APIs (Step 7)
...
Next step: Skill \`9-review-frontend\`."
```
